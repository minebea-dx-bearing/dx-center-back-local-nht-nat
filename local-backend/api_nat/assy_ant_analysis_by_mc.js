...
const DATABASE_STATUS = "[nat_mc_assy_ant_new].[dbo].[DATA_MCSTATUS_ANT]";
...
router.get("/production_hour_by_mc/:mc_no/:date", async (req, res) => {
  try {
    ...
    if(date < '2026-09-08'){
      calc = (mc % 2 === 0) ? mc/2 : (mc+1)/2
      calc_mc_no = mc_no.slice(0,3) + String(calc).padStart(2, '0')
    } else {
      calc_mc_no = mc_no
    }
    // console.log(mc_no, calc_mc_no)
...
router.get("/status/:mc_no/:date", async (req, res) => {
  try {
    ...
    let data = await dbms.query(`
      DECLARE @start_date DATETIME = '${date} 06:00'; -- เปลี่ยนวันที่ด้วย
      DECLARE @end_date DATETIME = '${dateTomarrow} 06:00'; -- เปลี่ยนวันที่ด้วย
...
router.get("/alarm/:mc_no/:date", async (req, res) => {
  try {
    let { mc_no, date } = req.params;
    let dateTomarrow = moment(date).add(1, "day").endOf("day").format("YYYY-MM-DD");
...
router.get("/production_hour_all_mc/:date", async (req, res) => {
  try {
    ...
    if(date < '2026-09-08'){
      mc_no = {
        front: "LEFT([mc_no], 3) + RIGHT('0' + CONVERT(VARCHAR(10), (CONVERT(INT, RIGHT([mc_no], 2)) * 2)),2) AS [mc_no]",
        rear: "LEFT([mc_no], 3) + RIGHT('0' + CONVERT(VARCHAR(10), CONVERT(INT, RIGHT([mc_no], 2)) + (CONVERT(INT, RIGHT([mc_no], 2)) - 1)),2) AS [mc_no]"
      }
      condition = {
        front: "",
        rear: ""
      ...
      condition = {
        front: " AND CAST(RIGHT([mc_no], 1) AS INT) % 2 = 0",
        rear: " AND CAST(RIGHT([mc_no], 1) AS INT) % 2 = 1"
      }
    }
    ...
    let data = await dbms.query(`
      WITH [data] AS (
        ...
        WHERE (FORMAT(IIF(DATEPART(HOUR, [registered]) < 7, DATEADD(DAY, -1, [registered]), [registered]), 'yyyy-MM-dd') = '${date}' 
		    OR (FORMAT([registered], 'yyyy-MM-dd hh') = '${date} 06')) ${condition.front}
        UNION
        SELECT [registered],
            convert(varchar, [registered], 8) AS TIME,
            format(iif(DATEPART(HOUR, [registered]) < 7, dateadd(DAY, -1, [registered]), [registered]), 'yyyy-MM-dd') AS [mfg_date],
        ...
        FROM ${DATABASE_PROD}
        WHERE (FORMAT(IIF(DATEPART(HOUR, [registered]) < 7, DATEADD(DAY, -1, [registered]), [registered]), 'yyyy-MM-dd') = '${date}' 
		    OR (FORMAT([registered], 'yyyy-MM-dd hh') = '${date} 06')) ${condition.rear}
      )
      SELECT
        *,
