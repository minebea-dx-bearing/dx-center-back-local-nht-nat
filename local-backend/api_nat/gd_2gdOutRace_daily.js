...
router.post("/data", async (req, res) => {
  ...
  if (mcNoQuery === "ALL") {
    queryWhere1 = "";
    queryWhere2 = "WHERE [mc_no] LIKE 'OR%R'";
  } else {
    queryWhere1 = `AND [mc_no] = '${mcNoQuery}'`;
    queryWhere2 = `WHERE [mc_no] LIKE 'OR%R' AND [mc_no] = '${mcNoQuery}'`;
  }
  ...
  try {
    const response_prod = await dbms.query(
      `
        ...
        WHERE [sum_data].[mc_no] LIKE 'OR%R'
      `
    );
      ...
      `
        ...
        WITH [base_alarm] AS (
            ...
            SELECT
                ...
                END AS [alarm_type]
            FROM [nat_mc_mcshop_2gd].[dbo].[DATA_ALARMLIS_2GD]
            WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND [mc_no] LIKE 'OR%R'
        ),
        [with_pairing] AS (
