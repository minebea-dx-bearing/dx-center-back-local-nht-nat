  ...
  try {
    let master = await dbms.query(
      `
        ...
        FROM ${DATABASE_PROD}
        WHERE [mc_no] LIKE 'IR%H' -- มีเฉพาะเครื่อง 2nd In Super
        ORDER BY mc_no ASC
      `
    );
...
router.get("/production_hour_by_mc/:mc_no/:date", async (req, res) => {
  try {
    ...
    const result = await getData.productionByHour(DATABASE_PROD, COLUMN_OK, COLUMN_NG, COLUMN_TOTAL, COLUMN_CT, mc_no, date)
    res.json(result);
  } catch (error) {
    res.status(500).json({ data: [], success: false, message: "Internal Server Error" });
  }
