    ...
    const response_prod = await dbms.query(
      `
        ...
        WITH cte AS (
            ...
            [shield_ok],
            LAG([shield_ok]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_shield_ok],
            [shield_a_ng],
            LAG([shield_a_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_shield_a_ng],
            [shield_b_ng],
            LAG([shield_b_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_shield_b_ng],
            [snap_a_ng],
            LAG([snap_a_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_snap_a_ng],
            [snap_b_ng],
            LAG([snap_b_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_snap_b_ng]
            FROM [nat_mc_assy_gssm].[dbo].[DATA_PRODUCTION_GSSM]
        ...
        [sum_data] AS (
            SELECT
                ...
                SUM(
                CASE 
                  WHEN [prev_shield_ok] IS NULL THEN 0
                  WHEN [shield_ok] < [prev_shield_ok] THEN [shield_ok]
                  ELSE [shield_ok] - [prev_shield_ok]
                END
                ) AS [total_shield_ok],
                SUM(
                CASE 
                  WHEN [prev_shield_a_ng] IS NULL THEN 0
                  WHEN [shield_a_ng] < [prev_shield_a_ng] THEN [shield_a_ng]
                  ELSE [shield_a_ng] - [prev_shield_a_ng]
                END
                ) AS [total_shield_a_ng],
                SUM(
                CASE 
                  WHEN [prev_shield_b_ng] IS NULL THEN 0
                  WHEN [shield_b_ng] < [prev_shield_b_ng] THEN [shield_b_ng]
                  ELSE [shield_b_ng] - [prev_shield_b_ng]
                END
                ) AS [total_shield_b_ng],
                SUM(
                CASE 
                  WHEN [prev_snap_a_ng] IS NULL THEN 0
                  WHEN [snap_a_ng] < [prev_snap_a_ng] THEN [snap_a_ng]
                  ELSE [snap_a_ng] - [prev_snap_a_ng]
                END
                ) AS [total_snap_a_ng],
                SUM(
                CASE 
                  WHEN [prev_snap_b_ng] IS NULL THEN 0
                  WHEN [snap_b_ng] < [prev_snap_b_ng] THEN [snap_b_ng]
                  ELSE [snap_b_ng] - [prev_snap_b_ng]
                END
                ) AS [total_snap_b_ng]
              FROM cte
        ...
        SELECT
            [sum_data].*,
            [sum_data].[total_shield_ok] + [sum_data].[total_shield_a_ng] + [sum_data].[total_shield_b_ng] + [sum_data].[total_snap_a_ng] + [sum_data].[total_snap_a_ng] AS [total_prod],
	          [sum_data].[total_shield_ok] AS [total_daily_ok],
