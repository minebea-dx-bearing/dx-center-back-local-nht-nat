...
router.post("/data", async (req, res) => {
  ...
  if (mcNoQuery === "ALL") {
    queryWhere1 = "";
    queryWhere2 = "WHERE [mc_no] LIKE 'IR%H'";
  } else {
    queryWhere1 = `AND [mc_no] = '${mcNoQuery}'`;
    queryWhere2 = `WHERE [mc_no] LIKE 'IR%H' AND [mc_no] = '${mcNoQuery}'`;
  }
  ...
  try {
    const response_prod = await dbms.query(
      `
        ...
        WHERE [sum_data].[mc_no] LIKE 'IR%H'
      `
    );
      ...
      `
        ...
        WITH [base_alarm] AS (
            ...
            SELECT
                ...
                END AS [alarm_type]
            FROM [nat_mc_mcshop_2gd].[dbo].[DATA_ALARMLIS_2GD]
            WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND [mc_no] LIKE 'IR%H'
        ),
        [with_pairing] AS (
