...
router.post("/data", async (req, res) => {
  ...
  if (mcNoQuery === "ALL") {
    queryWhere1 = "";
    queryWhere2 = "WHERE [mc_no] LIKE 'IR%B'";
  } else {
    queryWhere1 = `AND [mc_no] = '${mcNoQuery}'`;
    queryWhere2 = `WHERE [mc_no] LIKE 'IR%B' AND [mc_no] = '${mcNoQuery}'`;
  }
    ...
    const response_prod = await dbms.query(
      `
        ...
        WITH cte AS (
          ...
          [tng],
            LAG([tng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_tng]
        ...
        SELECT
            [sum_data].*,
            [sum_data].[total_prod_total] + [sum_data].[total_tng] AS [total_prod],
            [sum_data].[total_tng] AS [total_ng],
        ...
        WHERE [sum_data].[mc_no] LIKE 'IR%B'
      `
    );
      ...
      `
        ...
        WITH [base_alarm] AS (
            ...
            SELECT
                ...
                END AS [alarm_type]
            FROM [nat_mc_mcshop_2gd].[dbo].[DATA_ALARMLIS_2GD]
            WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND [mc_no] LIKE 'IR%B'
        ),
        [with_pairing] AS (
