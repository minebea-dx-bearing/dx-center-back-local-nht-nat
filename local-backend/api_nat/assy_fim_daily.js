    ...
    const response_prod = await dbms.query(
      `
        ...
        WITH cte AS (
            ...
            [fim_ok],
            LAG([fim_ok]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_fim_ok],
            [id_ng],
            LAG([id_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_id_ng],
            [od_ng],
            LAG([od_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_od_ng],
            [width_ng],
            LAG([width_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_width_ng],
            [chamfer_ng],
            LAG([chamfer_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_chamfer_ng],
            [mix_ng],
            LAG([mix_ng]) OVER (PARTITION BY [mc_no] ORDER BY [registered]) AS [prev_mix_ng]
            FROM [nat_mc_assy_fim].[dbo].[DATA_PRODUCTION_FIM]
        ...
        [sum_data] AS (
            SELECT
                ...
                SUM(
                CASE 
                  WHEN [prev_fim_ok] IS NULL THEN 0
                  WHEN [fim_ok] < [prev_fim_ok] THEN [fim_ok]
                  ELSE [fim_ok] - [prev_fim_ok]
                END
                ) AS [total_fim_ok],
                SUM(
                CASE 
                  WHEN [prev_id_ng] IS NULL THEN 0
                  WHEN [id_ng] < [prev_id_ng] THEN [id_ng]
                  ELSE [id_ng] - [prev_id_ng]
                END
                ) AS [total_id_ng],
                SUM(
                CASE 
                  WHEN [prev_od_ng] IS NULL THEN 0
                  WHEN [od_ng] < [prev_od_ng] THEN [od_ng]
                  ELSE [od_ng] - [prev_od_ng]
                END
                ) AS [total_od_ng],
                SUM(
                CASE 
                  WHEN [prev_width_ng] IS NULL THEN 0
                  WHEN [width_ng] < [prev_width_ng] THEN [width_ng]
                  ELSE [width_ng] - [prev_width_ng]
                END
                ) AS [total_width_ng],
                SUM(
                CASE 
                  WHEN [prev_chamfer_ng] IS NULL THEN 0
                  WHEN [chamfer_ng] < [prev_chamfer_ng] THEN [chamfer_ng]
                  ELSE [chamfer_ng] - [prev_chamfer_ng]
                END
                ) AS [total_chamfer_ng],
                SUM(
                CASE 
                  WHEN [prev_mix_ng] IS NULL THEN 0
                  WHEN [mix_ng] < [prev_mix_ng] THEN [mix_ng]
                  ELSE [mix_ng] - [prev_mix_ng]
                END
                ) AS [total_mix_ng]
              FROM cte
        ...
        SELECT
            [sum_data].*,
            [sum_data].[total_fim_ok] + [sum_data].[total_id_ng] + [sum_data].[total_od_ng] + [sum_data].[total_width_ng] + [sum_data].[total_chamfer_ng] + [sum_data].[total_mix_ng] AS [total_prod],
            [sum_data].[total_fim_ok] AS [total_daily_ok],
    ...
    const response_alarm = await dbms.query(
      `
        ...
        WITH [base_alarm] AS (
            ...
            SELECT
                ...
                CASE
                    ...
                    ELSE 'before'
                END AS [alarm_type]
            FROM [nat_mc_assy_fim].[dbo].[DATA_ALARMLIS_FIM]
        ...
        [base_monitor_iot] AS (
            ...
            FROM [nat_mc_assy_fim].[dbo].[MONITOR_IOT]
