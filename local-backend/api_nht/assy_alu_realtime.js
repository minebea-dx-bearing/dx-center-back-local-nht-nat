...
const DATABASE_ALARM = `[data_machine_${processName.toLowerCase()}].[dbo].[DATA_ALARMLIS_${processName.toUpperCase()}]`;
...
const prepareRealtimeData = (currentMachineData, runningTimeData) => {
  return Object.values(currentMachineData).map((item) => {
    ...
    const act_pd = item.prod_cnt_qty || 0;
    const ng_pd = 0;
    const act_ct = item.cycletime / 10 || 0;
