...
const calculateShifts = (data, date) => {
  ...
  if (date === todayStr) {
    ...
    if (A_start && A_end) {
      ...
      const utl = (diff_total / (seconds / A_end.target_ct)) * 100 * A_end.ring_factor || 0.0;
  ...
  // -------------------------------------------------
    ...
    if (Mrow) {
      ...
      const utl = (Mrow.prod_total / (seconds / Mrow.target_ct)) * 100 * Mrow.ring_factor || 0.0;
    ...
    if (N_start && N_end) {
      ...
      const utl = (diff_total / (seconds / N_start.target_ct)) * 100 * N_start.ring_factor || 0.0;
      const ach = (diff_total / target_prod) * 100 || 0.0;
      const yieldVal = (diff_ok / diff_total) * 100 || 0.0;
    ...
    if (M && N) {
      ...
      const utl = (diff_total / (seconds / M.target_ct)) * 100 * M.ring_factor || 0.0;
