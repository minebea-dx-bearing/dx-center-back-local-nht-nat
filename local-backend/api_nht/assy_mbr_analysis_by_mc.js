...
const DATABASE_PROD = "[data_machine_assy1].[dbo].[DATA_PRODUCTION_ASSY]";
const DATABASE_ALARM = "[data_machine_assy1].[dbo].[DATA_ALARMLIS_ASSY]";
const DATABASE_IOT = "[data_machine_assy1].[dbo].[MONITOR_IOT]";
const DATABASE_MASTER = "[data_machine_assy1].[dbo].[DATA_MASTER_ASSY]";
...
const calculateShifts = (data, date) => {
  ...
  if (date === todayStr) {
    ...
    if (A_start && A_end) {
      ...
      const utl =
        (diff_total / ((seconds / A_end.target_ct) * A_end.ring_factor)) * 100;
  ...
  // -------------------------------------------------
    ...
    if (N_start && N_end) {
      ...
      const utl =
        (diff_total / ((seconds / N_start.target_ct) * N_start.ring_factor)) *
        100;
      const ach = (diff_total / target_prod) * 100;
      const yieldVal = (diff_ok / diff_total) * 100;
