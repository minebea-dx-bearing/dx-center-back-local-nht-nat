...
const productionByHour = async (dbms, DATABASE_PROD, COLUMN_OK, COLUMN_NG, COLUMN_TOTAL, COLUMN_CT, mc_no, date) => {
    try {
...
const status = async (dbms, DATABASE_PROD, DATABASE_STATUS, DATABASE_IOT, mc_no, date) =>{
    try {
      let dateTomarrow = moment(date).add(1, "day").endOf("day").format("YYYY-MM-DD");
...
const productionDaily = async(dbms, DATABASE_PROD, DATABASE_MASTER, COLUMN_TOTAL, COLUMN_OK, COLUMN_NG, mc_no, date) => {
  try{
    const data = await dbms.query(`
      ...
      SELECT 
        d.*,
          ...
          [ring_factor]
      FROM [data] d
      INNER JOIN [master] m ON d.[mc_no] = m.[mc_no] COLLATE Thai_CI_AS
